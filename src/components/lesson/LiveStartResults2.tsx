...
export function LiveStartResults2({ lessonSlug, sessionId }: Props) {
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-6 md:p-8 space-y-6">
      ...
      <ResultBlock title="Motywacja do nauki" data={sortedEntries(motivation)} total={total} />
      <ResultBlock title="Znane metody uczenia się" data={sortedEntries(methods)} total={total} />
      <ResultBlock title="Jak najczęściej się uczysz?" data={sortedEntries(currentMethod)} total={total} />
      <ResultBlock title="Czas nauki dziennie" data={sortedEntries(studyTime)} total={total} />
