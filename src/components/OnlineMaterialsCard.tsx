  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-xs font-semibold tracking-[0.15em] uppercase text-primary">Wkrótce</span>
      </div>

      <div className="grid md:grid-cols-2 gap-8 items-start">
        <div>
          <h3 className="font-heading text-2xl font-semibold mb-4">Rozszerzone materiały online</h3>
          <p className="text-muted-foreground leading-relaxed mb-6">
            Kompletny zestaw nagrań wideo, rozbudowanych ćwiczeń i zaawansowanych promptów — dostępny w formie płatnej
            subskrypcji. Idealne uzupełnienie warsztatów lub samodzielne źródło wiedzy o AI dla uczniów.
          </p>
        ...
        <div className="bg-card border border-border rounded-lg p-6">
          {formState === "success" ? (
            <div className="flex flex-col items-center text-center py-4 gap-3">
              ...
              <p className="text-sm text-muted-foreground">Dam Ci znać, gdy materiały będą gotowe.</p>
            </div>
          ) : (
            <>
              <p className="text-sm font-semibold mb-1">Zostaw swój email</p>
              <p className="text-xs text-muted-foreground mb-5">Powiadomię Cię, gdy materiały będą dostępne.</p>
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                >
                  {formState === "loading" ? <Loader2 className="w-4 h-4 animate-spin" /> : "Zapisz się na listę"}
                </button>
              </form>
            </>
          )}
