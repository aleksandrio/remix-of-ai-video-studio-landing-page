    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 grid md:grid-cols-[280px_1fr] gap-6">
        ...
        <div className="space-y-4">
          <h2 className="font-heading text-sm font-bold uppercase tracking-editorial text-muted-foreground">Lekcje</h2>
            ...
            {lessons.map((l) => (
              <div
                ...
                className={`p-3 rounded-lg border cursor-pointer transition-all ${
                  selectedSlug === l.lesson_slug
                    ? 'border-primary bg-primary/5'
                    : 'border-border hover:border-primary/50'
                }`}
                onClick={() => {
                  setSelectedSlug(l.lesson_slug)
              ...
              >
                ...
                <button
                  onClick={(e) => { e.stopPropagation(); deleteLesson(l.lesson_slug) }}
          ...
          <div className="border border-border rounded-lg p-3 space-y-2">
            <p className="text-xs font-bold uppercase tracking-editorial text-muted-foreground">Nowa lekcja</p>
            <input
              placeholder="Slug (np. ai-intro)"
              value={newSlug}
              onChange={(e) => setNewSlug(e.target.value.replace(/[^a-z0-9-]/g, ''))}
              className="w-full border border-border bg-background text-foreground rounded p-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
            />
            <input
              placeholder="Tytuł"
              value={newTitle}
            ...
            <button
              ...
              className="w-full py-2 bg-primary text-primary-foreground text-sm font-semibold rounded disabled:opacity-50 hover:opacity-90 transition-all"
            >
              Dodaj
            </button>
          </div>
            ...
            <>
              ...
              <div className="bg-card border border-border rounded-lg p-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <a
                    href={`/lekcja/${selectedSlug}`}
                    target="_blank"
                    rel="noopener"
                    className="text-xs text-primary hover:underline"
                  >
                    Otwórz stronę ucznia →
                  </a>
                </div>
                  ...
                  <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="text-xs border border-border px-3 py-1 rounded hover:bg-muted transition-all"
                      >
                        Archiwizuj
                      </button>
                    </div>
                ...
                <div className="flex gap-2">
                  <input
                    placeholder="Nazwa nowej sesji (np. 8A 2026-03-04)"
                    value={newSessionName}
                    onChange={(e) => setNewSessionName(e.target.value)}
                    className="flex-1 border border-border bg-background text-foreground rounded p-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                  />
                  <button
                    onClick={createSession}
                    className="px-4 py-2 bg-primary text-primary-foreground text-sm font-semibold rounded hover:opacity-90 transition-all"
                  >
                    Nowa sesja
                  </button>
                </div>
                ...
                {archivedSessions.length > 0 && (
                  <details className="text-sm">
                    <summary className="cursor-pointer text-muted-foreground hover:text-foreground">
                      Archiwalne sesje ({archivedSessions.length})
                    </summary>
                      ...
                      {archivedSessions.map((s) => (
                        <div
                          ...
                          className={`flex justify-between items-center p-2 rounded cursor-pointer transition-all ${
                            selectedSessionId === s.id ? 'bg-muted' : 'hover:bg-muted/50'
                          }`}
                        ...
                        >
                          ...
                          <span className="text-xs text-muted-foreground">
                            {s.archived_at ? new Date(s.archived_at).toLocaleDateString('pl') : ''}
                          </span>
                        </div>
                      ))}
                    </div>
              ...
              {feedbackResponses.length > 0 && (surveyTypeFilter === '' || surveyTypeFilter === 'feedback') && (
                <div className="bg-card border border-border rounded-lg p-6">
                  <h3 className="font-heading text-sm font-bold uppercase tracking-editorial text-muted-foreground mb-4">
                  ...
                  <div key={r.id} className="bg-card border border-border rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-2">
                      <span className={`text-xs font-bold uppercase px-2 py-0.5 rounded ${
                        r.survey_type === 'start' ? 'bg-primary/10 text-primary' : 'bg-accent-terracotta/10 text-accent-terracotta'
                      }`}>
                        {r.survey_type}
                      </span>
                      <span className="text-xs text-muted-foreground">
                        {new Date(r.created_at).toLocaleString('pl')}
                      </span>
                    </div>
